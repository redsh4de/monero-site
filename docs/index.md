# Contributor Documentation Index

## Getting Started

Essential guides for new contributors to get set up.

- [How to Set Up the Development Environment](how-to-set-up-the-development-environment.md)
- [Understanding the Project Structure](understanding-the-project-structure.md)

## Page Development

Guides for building and customizing pages, including creation, styling, components, and localization.

- [How to make a new page](how-to-make-a-new-page.md)
- [How to style your page contents](how-to-style-your-page-contents.md)
- [How to use the UI library](how-to-use-the-ui-library.md)
- [How to handle assets and images](how-to-handle-assets-and-images.md)
- [How to use and create icons](how-to-use-and-create-icons.md)
- [How to create a new component](how-to-create-a-new-component.md)

## Localization

- [How to add localization to a page](how-to-add-localization-to-a-page.md)
- [How to configure a new language](how-to-configure-a-new-language.md)

## Content Creation

Guides for creating new content on the site, such as blog posts, Moneropedia entries, and FAQs.

- [How to make a blog post](how-to-make-a-blog-post.md)
- [How to make a Moneropedia entry](how-to-make-a-moneropedia-entry.md)
- [How to add a question to the FAQ page](how-to-add-a-question-to-the-faq-page.md)

## Content Enhancement

Guides for enhancing existing content.

- [How to link to Moneropedia terms](how-to-link-to-moneropedia-terms.md)

If you need a guide that's not listed here, please create a [issue](https://github.com/monero-project/monero-site/issues) or contact the Website workgroup.